...
const inputTitle = ref(phobia.title)
const input = ref(phobia.md)
...
function saveMd(e) {
    ...
    try {
        router.post('/phobia/' + phobia.id, { md: input.value, title: inputTitle.value })
    } catch (error) {
        console.log(error)
    }
}
...
function deleteMd(e) {          //TODO
    console.log(input.value)
    input.value = ''
    inputTitle.value = ''
    router.delete('/phobia/' + phobia.id)
}
